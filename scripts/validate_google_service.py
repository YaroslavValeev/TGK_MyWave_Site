...
def main():
    p = argparse.ArgumentParser(description='Validate Google service account JSON and optionally test Sheets access')
    p.add_argument('--key', '-k', required=True, help='Path to service_account.json')
    p.add_argument('--spreadsheet', '-s', required=False, help='Spreadsheet ID to test access')
    args = p.parse_args()
    ...
    for req in ('type', 'client_email', 'private_key'):
        if req not in info:
            print(f'ERROR: required field "{req}" missing in service account JSON', file=sys.stderr)
            sys.exit(4)
    ...
    if args.spreadsheet:
        ...
        try:
            ...
            print('ERROR: google libraries not available (google-auth/google-api-python-client).', file=sys.stderr)
            print('Install with: pip install google-auth google-api-python-client', file=sys.stderr)
            sys.exit(5)
        ...
        try:
            ...
            if 'invalid_grant' in str(e) or 'invalid_signature' in str(e) or 'invalid' in str(e).lower():
                print('Hint: invalid_grant / invalid signature typically means the private_key or timezone is wrong, or the service account json is malformed.', file=sys.stderr)
            sys.exit(6)
