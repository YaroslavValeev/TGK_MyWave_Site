    ...
    for i, line in enumerate(lines, start=1):
        ...
        if mcf:
            ...
            if i-2 >= 0:
                prev = lines[i-2]
                if prev.strip() != '':
                    issues.append((str(md), i, 'MD031', 'Code fence should be preceded by a blank line'))
            if lang == '':
                issues.append((str(md), i, 'MD040', 'Fenced code block should have a language specified'))
            # find closing fence
            j = i
            found_close = False
            ...
            if found_close:
                ...
                if j+1 < len(lines) and lines[j+1].strip() != '':
                    issues.append((str(md), j+1, 'MD031', 'Code fence should be followed by a blank line'))
            continue
        ...
        if mh:
            ...
            if prev_level and level > prev_level + 1:
                issues.append((str(md), i, 'MD001', f'Heading level increased from {prev_level} to {level}'))
            prev_level = level
            key = (level, text)
            if key in headings_seen:
                issues.append((str(md), i, 'MD024', f'Duplicate heading "{mh.group(2).strip()}" previously at line {headings_seen[key]}'))
            else:
                headings_seen[key] = i
            continue
        # emphasis as heading
...
if total == 0:
    ...
    print('Please review the above warnings. For structural fixes (MD001/MD024/MD036) confirm before auto-fix.')
