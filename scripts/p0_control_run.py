    ...
    with app.app_context():
        ...
        for record in records:
            ...
            if (
                ...
                and published not in ("TRUE", "1")
                and row_num
                and isinstance(row_num, (int, str))
                and str(row_num).strip()
                and int(str(row_num).strip()) >= 2
            ):
                test_record = record
                break
        ...
        slug = test_record.get("slug", "") or test_record.get(
            "raw_title", ""
        ).lower().replace(" ", "-")
        ...
        checks = {
            ...
            "canonical_url правильный формат": (
                after["canonical_url"].startswith("https://mywavetreaning.ru/blog/")
                if after["canonical_url"]
                else False
            ),
            "publish_error пуст": not after["publish_error"]
            or after["publish_error"].strip() == "",
        }
...
def test_2_row_number_missing():
    ...
    with app.app_context():
        ...
        for record in records:
            ...
            is_invalid = (
                not row_num
                or str(row_num).strip() == ""
                or (
                    isinstance(row_num, (int, str))
                    and (
                        not str(row_num).strip().isdigit()
                        or int(str(row_num).strip()) < 2
                    )
                )
            )
        ...
        # Проверки
        checks = {
            "published_posts НЕ изменился": after["published_posts"]
            == before["published_posts"],
            "published_at НЕ заполнен": not after["published_at"]
            or after["published_at"] == before["published_at"],
            "canonical_url НЕ заполнен": not after["canonical_url"]
            or after["canonical_url"] == before["canonical_url"],
            "publish_error = WP_ROW_NUMBER_MISSING или WP_ROW_NUMBER_INVALID": (
                "WP_ROW_NUMBER_MISSING" in str(after["publish_error"])
                or "WP_ROW_NUMBER_INVALID" in str(after["publish_error"])
            ),
