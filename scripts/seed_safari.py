...
with app.app_context():
    ...
    if not p:
        p = Participant(name='Alice', email='alice@example.com', phone='+79123456789', level='intermediate')
        db.session.add(p)
        db.session.commit()
    ...
    if not b:
        ...
        b = SafariBooking(participant_id=p.id, status='confirmed', start_date=date.today(), days=3, message='Seed booking')
        db.session.add(b)
        db.session.commit()
