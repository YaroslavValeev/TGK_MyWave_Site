...
def main():
            ...
            try:
                ...
                if image_count > 0:
                    all_passed &= print_check(
                        f"Found {image_count} service images", True
                    )
                else:
                    print(
                        "⚠️  No service images found. Run: python scripts/seed_service_images.py"
                    )
            except Exception as e:
    ...
    for method, endpoint, desc in endpoints_to_test:
        try:
            ...
            all_passed &= print_check(
                f"{method:4} {endpoint:35} → {resp.status_code}", True
            )
            ...
            all_passed &= print_check(
                f"{method:4} {endpoint:35} → ERROR: {str(e)[:40]}", False
            )
        ...
        with app.app_context():
            ...
            try:
                ...
                if service:
                    ...
                    print(
                        "⚠️  Google Sheets service not available (may be OK in dev mode)"
                    )
            except Exception as e:
    ...
    try:
        ...
        if csp_header:
            ...
            all_passed &= print_check(
                f"CSP header present ({len(csp_header)} chars)", True
            )
            ...
            if not unsafe_inline:
                ...
                print(
                    "⚠️  WARNING: 'unsafe-inline' detected in CSP (reduce XSS protection)"
                )
        else:
    ...
    try:
        with app.app_context():
            flags = {
                "ENABLE_RECOMMENDATIONS": app.config.get(
    ...
    if all_passed:
        ...
        print("\nNext steps:")
        print(
            "  1. If no service images found, run: python scripts/seed_service_images.py"
        )
        print("  2. Test analytics: python scripts/test_analytics_log.py")
        print(
            "  3. Monitor CSP violations: Check GET /api/csp-violations in browser console"
        )
