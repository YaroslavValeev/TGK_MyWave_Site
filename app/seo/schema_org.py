...
def build_item_list(showcases: Iterable[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    for idx, showcase in enumerate(showcases, start=1):
        ...
        url = (
            showcase.get("absolute_url")
            or showcase.get("url")
            or url_for("projects_page", _external=True)
        )
        elements.append(
            {
                "@type": "ListItem",
...
def build_showcase_schema(showcase: Mapping[str, Any]) -> dict[str, Any]:
        ...
        additional_properties.append(
            {
                "@type": "PropertyValue",
                "name": "tags",
                "value": ", ".join(_normalize_tags(showcase.get("tags"))),
            }
        )
    if showcase.get("level"):
        additional_properties.append(
            {"@type": "PropertyValue", "name": "level", "value": showcase["level"]}
        )
    ...
    if showcase.get("city"):
        location = {
            "@type": "Place",
            "name": showcase["city"],
            "address": f"{showcase.get('city')}, {showcase.get('country', '')}".strip(
                ", "
            ),
        }
    schema = {
        ...
        "eventStatus": showcase.get("eventStatus")
        or "https://schema.org/EventScheduled",
        "eventAttendanceMode": showcase.get("eventAttendanceMode")
        or "https://schema.org/OfflineEventAttendanceMode",
...
def build_showcase_graph(showcases: list[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "@graph": [
            build_item_list(showcases),
            *[build_showcase_schema(sc) for sc in showcases],
        ],
    }
