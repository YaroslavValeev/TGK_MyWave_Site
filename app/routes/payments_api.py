...
payment_status_model = api.model(
    "PaymentStatus",
    {
        ...
        "status": fields.String(
            required=True, description="pending|succeeded|canceled|failed"
        ),
        "amount": fields.Float(),
        ...
        api.model(
            "PaymentRequest",
            {
                ...
                "description": fields.String(
                    required=True, description="Payment description"
                ),
                "return_url": fields.String(
                    required=True, description="Redirect URL after payment"
                ),
    ...
    def post(self):
        ...
        try:
            ...
            if not all(k in data for k in ["amount", "description", "return_url"]):
                api.abort(
                    400, "Missing required fields: amount, description, return_url"
                )
...
class PaymentRefund(Resource):
    ...
    @api.expect(
        api.model(
            ...
            {
                "payment_id": fields.String(
                    required=True, description="Payment ID to refund"
                ),
    ...
    def post(self):
        ...
        try:
            ...
            refund = processor.refund_payment(
                payment_id=data["payment_id"], amount=data.get("amount")
            )

            logger.info(
                f"Refund created: {refund['refund_id']} for payment {data['payment_id']}"
            )
            return refund, 201
