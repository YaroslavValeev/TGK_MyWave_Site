...
def about_page():
    ...
    if os.path.exists(images_dir):
        images += [f'about/{f}' for f in os.listdir(images_dir) if f.lower().endswith(('.jpg', '.jpeg', '.png', '.webp', '.gif'))]
    ...
    if os.path.exists(about_videos_dir):
        videos = [f for f in os.listdir(about_videos_dir) if f.lower().endswith(('.mp4', '.webm', '.mov', '.avi'))]
