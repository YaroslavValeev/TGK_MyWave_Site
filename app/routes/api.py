        ...
        for client in clients:
            if client.get("phone") == phone:
                current_app.logger.info(
                    f"Найден существующий клиент: {client.get('client_id')}"
                )
    ...
    try:
        ...
        if not spreadsheet_id:
            current_app.logger.warning(
                "SPREADSHEET_ID не настроен, возвращаем броней из локальной БД"
            )
        ...
        try:
            ...
            current_app.logger.info(
                f"Загружено {len(client_workouts)} броней из Google Sheets"
            )
            ...
            current_app.logger.error(
                f"Ошибка Google API при получении броней: {error_msg}"
            )
...
def api_bookings_create():
    ...
    try:
        ...
        if not spreadsheet_id:
            current_app.logger.warning(
                "SPREADSHEET_ID не настроен, сохраняем только в локальную БД"
            )
            booking = Booking(
                name=name,
                phone=phone,
        ...
        if not workout_id:
            ...
            try:
                ...
                current_app.logger.info(
                    f"Новая тренировка создана в листе Workouts: {workout_id} на {date_str} {time_str}"
                )
            except Exception as e:
                current_app.logger.error(
                    f"Ошибка создания тренировки в Workouts: {str(e)}"
                )
        ...
        try:
            ...
            current_app.logger.info(
                f"Бронь записана в Google Sheets: {client_id} на {date_str} {time_str}"
            )
        except HttpError as he:
            error_msg = str(he)
            ...
            if "invalid_grant" in error_msg:
                return jsonify({"error": "Ошибка авторизации сервера"}), 503
            return (
                jsonify({"error": "Не удалось сохранить бронирование в Google Sheets"}),
                502,
            )
        except Exception as e:
        ...
        try:
            ...
            calendar_created = add_event_to_calendar(
                service, date_str, time_str, name, phone
            )
            if calendar_created:
                current_app.logger.info(
                    f"Событие добавлено в Google Calendar: {name} на {date_str} {time_str}"
                )
            else:
                current_app.logger.warning(
                    f"Не удалось добавить событие в Google Calendar (non-blocking)"
                )
        except Exception as e:
            current_app.logger.warning(
                f"Ошибка добавления события в Calendar (non-blocking): {str(e)}"
            )
        ...
        try:
            ...
            local_id = booking.id
            current_app.logger.info(
                f"Бронь синхронизирована в локальную БД: ID={local_id}"
            )
        except Exception as e:
            models_db.session.rollback()
            current_app.logger.warning(
                f"Не удалось синхронизировать в локальную БД: {str(e)}"
            )
            local_id = None
            ...
            jsonify(
                {
                    ...
                    "calendar_event_created": (
                        calendar_created if "calendar_created" in locals() else False
                    ),
                }
            ),
            201,
        ...
        return (
            jsonify({"error": "Ошибка валидации данных", "details": ve.messages}),
            400,
        )
...
def api_lead():
    ...
    try:
        ...
        if lead_type not in ("coach_trip", "consulting", "camp"):
            return (
                jsonify({"error": "type должен быть coach_trip, consulting или camp"}),
                400,
            )
...
def get_knowledge(type):
    ...
    if type == "training":
        ...
        tricks_path = os.path.join(
            base_path, "tricks.txt", "Список трюков по вейксерфу.txt"
        )
        try:
