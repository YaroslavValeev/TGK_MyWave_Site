...
from app.services.projects.wsc2025_service import (
    save_participant_registration,
    save_coach_registration,
)
...
def _split_markdown_by_sections(md_text: str, anchors: list) -> dict:
    ...
    for match in re.finditer(header_pattern, html, re.IGNORECASE | re.DOTALL):
        ...
        header_text = re.sub(
            r"<[^>]+>", "", match.group(3)
        ).strip()  # Убираем HTML теги из текста
        headers.append(
            {
                "level": level,
                "id": header_id,
...
def project_page():
    ...
    menu = _read_json(
        base / "menu.json",
        {
            ...
            "downloads": [
                {
                    "label": "📄 Материалы участника",
                    "href": "/static/docs/wsc_participant_pack.zip",
                },
                {
                    "label": "📄 Материалы тренера",
                    "href": "/static/docs/wsc_coach_pack.zip",
                },
                {
                    "label": "📄 Пакет спонсора",
                    "href": "/static/docs/wsc_sponsor_pack.zip",
                },
            ],
        },
    )
    ...
    sponsor = _read_json(
        base / "sponsor_packages.json",
        {
            "packages": [
                {
                    "tier": "Бронза",
                    "price": 100000,
                    "deliverables": ["Логотип на сайте"],
                },
                {"tier": "Серебро", "price": 500000, "deliverables": ["Все из Бронзы"]},
                {
                    "tier": "Золото",
                    "price": 1000000,
                    "deliverables": ["Все из Серебра"],
                },
            ],
            "currency": "₽",
    ...
    judging = _read_json(
        base / "judging_criteria.json", {"outliers": {"threshold": 2.0}}
    )

    schema_event = _read_json(
    ...
    return render_template(
        ...
        project_html=index_md
        and md.markdown(index_md, extensions=["extra", "sane_lists"])
        or "",
...
@wakesurf_challenge_bp.route(
    "/wakesurf-challenge-2025/api/participants/register", methods=["POST"]
)
    ...
    try:
        ...
        try:
            ...
            send_telegram_notification(
                form_data["full_name"], form_data["phone"], notification_text
            )
...
@wakesurf_challenge_bp.route(
    "/wakesurf-challenge-2025/api/coaches/register", methods=["POST"]
)
@limiter.limit("5 per minute", key_func=get_remote_address)
def register_coach():
    """
