    ...
    if request.method == "POST":
        if form.validate_on_submit():
            ...
            if not re.match(r"^\+7\d{10}$", phone):
                flash(
                    "Неверный номер телефона. Введите в формате +7XXXXXXXXXX", "danger"
                )
                return (
                    render_template(
                        "book.html", form=form, content=SUCCESS_VIEW_CONTENT
                    ),
                    400,
                )
            try:
                ...
                if exists:
                    flash(
                        "Вы уже записаны на этот слот. Если хотите изменить, напишите нам.",
                        "warning",
                    )
            ...
            return (
                render_template("book.html", form=form, content=SUCCESS_VIEW_CONTENT),
                400,
            )
        ...
        if exists:
            return (
                jsonify(
                    {
                        "success": False,
                        "error": "Вы уже записаны на этот слот. Если хотите изменить, напишите нам.",
                    }
                ),
                409,
            )
            ...
            jsonify(
                {
                    ...
                    "success_view_url": url_for(
                        "booking.booking_success_view", type=service
                    ),
                }
            ),
            200,
