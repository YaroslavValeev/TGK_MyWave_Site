...
if TELEGRAM_BOT_TOKEN:
    try:
        ...
        logging.getLogger(__name__).exception(
            "Telegram Application init failed; continuing without telegram."
        )
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    response_text = ask(prompt=user_message, client_id=str(chat_id), source="telegram")
...
if application:
    try:
        ...
        logging.getLogger(__name__).exception(
            "Failed to init telegram handlers; continuing without telegram."
        )
