...
def get_slots(date_str):
    try:
        ...
        return (
            jsonify(
                {
                    "error": "Ошибка настройки сервера. Пожалуйста, обратитесь к администратору."
                }
            ),
            500,
        )
...
def _book_slot_internal():
    ...
    try:
        ...
        if not available_slot:
            ...
            return (
                jsonify({"status": "error", "error": "Слот недоступен или уже занят"}),
                400,
            )
        ...
        try:
            ...
            created = add_event_to_calendar(
                service, data["date"], data["time"], data["name"], data["phone"]
            )
            current_app.logger.info(
                f"Добавление события в календарь вернуло: {created}"
            )
        except Exception as e:
