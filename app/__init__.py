...
try:
    ...
    logging.getLogger(__name__).exception(
        "Failed to import telegram_bp; continuing without telegram support"
    )
...
def create_app(config_name="development"):
    ...
    if config_name.lower() != "production":
        if enable_google_env is None:
            ...
            app.config["ENABLE_GOOGLE_SERVICES"] = str(enable_google_env).lower() in (
                "1",
                "true",
                "yes",
            )
    ...
    raw_keys = os.getenv("AI_GATEWAY_API_KEYS") or os.getenv(
        "AI_GATEWAY_API_KEYS".lower()
    )
    if raw_keys:
        app.config["AI_GATEWAY_API_KEYS"] = [
    ...
    if require_key is None:
        ...
        app.config["AI_GATEWAY_REQUIRE_API_KEY"] = str(require_key).lower() in (
            "1",
            "true",
            "yes",
        )
    ...
    if rate_limit_enabled is None:
        ...
        app.config["AI_GATEWAY_ENABLE_RATE_LIMIT"] = str(
            rate_limit_enabled
        ).lower() in ("1", "true", "yes")
    # Rate limit params (count per window)
    try:
        app.config["AI_GATEWAY_RATE_LIMIT_COUNT"] = int(
            os.getenv("AI_GATEWAY_RATE_LIMIT_COUNT") or 60
        )
    except Exception:
        app.config["AI_GATEWAY_RATE_LIMIT_COUNT"] = 60
    try:
        app.config["AI_GATEWAY_RATE_LIMIT_WINDOW"] = int(
            os.getenv("AI_GATEWAY_RATE_LIMIT_WINDOW") or 60
        )
    except Exception:
    ...
    if telegram_bp:
        try:
            ...
            app.logger.exception(
                "Failed to register telegram blueprint; continuing without telegram"
            )
    ...
    try:
        ...
        try:
            from app.routes.ai_gateway_api import (
                ai_gateway_bp as _ai_bp,
                ai_safari_bp as _ai_safari_bp,
            )
            ...
            app.logger.debug(
                "Could not exempt ai_gateway_bp from CSRF (maybe not registered)"
            )
        try:
            ...
            app.logger.debug(
                "Could not exempt ai_concierge_bp from CSRF (maybe not registered)"
            )
        try:
            ...
            app.logger.debug(
                "Could not exempt safari_bp from CSRF (maybe not registered)"
            )
    except Exception:
    ...
    def analytics_log():
        ...
        try:
            ...
            sheet_id = app.config.get(
                "ANALYTICS_SHEET_SPREADSHEET_ID"
            ) or app.config.get("SPREADSHEET_ID")
            payload = {
                "event": event,
            ...
            if sheet_id:
                ...
                app.logger.warning(
                    "ANALYTICS_SHEET_SPREADSHEET_ID / SPREADSHEET_ID not configured; skipping sheet write"
                )
        except Exception:
            ...
            try:
                ...
                sheet_name = (
                    app.config.get("ANALYTICS_SHEET_NAME") or "analytics_statistics"
                )
                row = [
                    timestamp,
                    event,
                ...
                if sheet_id:
                    ...
                    app.logger.info(
                        f"Analytics logged to sheet {sheet_name} (fallback)"
                    )
                else:
                    app.logger.warning(
                        "ANALYTICS_SHEET_SPREADSHEET_ID / SPREADSHEET_ID not configured; skipping sheet write"
    ...
    with app.app_context():
        ...
        if not enable_google:
            app.logger.info(
                "Google services initialization is disabled (ENABLE_GOOGLE_SERVICES not set)."
            )
        else:
            ...
            try:
                ...
                if app.config.get("SPREADSHEET_ID"):
                    ...
                    try:
                        ...
                        app.logger.info(
                            "Successfully validated access to Google Sheets"
                        )
                    except Exception as e:
                        app.logger.error(f"Error accessing Google Sheet: {e}")
                        if "invalid_grant" in str(e):
                            app.logger.error(
                                "Invalid JWT Signature. Check your service_account.json file"
                            )
                ...
                if "invalid_grant" in str(e):
                    app.logger.error(
                        "Google Service Account authorization failed. Please check your service_account.json file"
                    )
