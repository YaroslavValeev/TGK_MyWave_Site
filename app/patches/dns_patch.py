...
def _getaddrinfo(*args: Any, **kwargs: Any) -> List[Tuple]:
    ...
    google_domains = ['.googleapis.com', '.google.com', 'oauth2.googleapis.com', 'sheets.googleapis.com']
    if any(domain in host for domain in google_domains):
            try:
                ...
                logger.error(f"Failed to resolve {host} using Google DNS ({type(e).__name__}): {e}")
                # Если это временная ошибка, пытаемся использовать системный DNS
                if isinstance(e, (dns.resolver.NoNameservers, dns.resolver.NoAnswer, dns.resolver.Timeout)):
    ...
    try:
        ...
        logger.info(f"Standard DNS resolution failed for {host}, trying alternative method")
        try:
