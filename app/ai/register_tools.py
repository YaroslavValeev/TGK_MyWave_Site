...
def register_safari_tools(gateway) -> None:
    ...
    gateway.register_tool("safari_create_booking", lambda payload: safari_create_booking(payload))
...
def register_challenge_tools(gateway) -> None:
    ...
    gateway.register_tool("challenge_get_rules", lambda p: get_rules(criteria=p.get("criteria")))
    gateway.register_tool("challenge_get_stage_deadlines", lambda p: get_stage_deadlines(stage=p.get("stage")))
    gateway.register_tool(
        "challenge_get_participant_info",
        lambda p: get_participant_info(participant_id=p.get("participant_id")),
    )
    gateway.register_tool("challenge_get_score", lambda p: get_score(participant_id=p.get("participant_id")))
    gateway.register_tool(
        "challenge_submit_score",
        lambda p: submit_score(judge_id=p.get("judge_id"), participant_id=p.get("participant_id")),
    )
    gateway.register_tool("challenge_get_results", lambda p: get_results(stage=p.get("stage")))
...
def register_sponsor_tools(gateway) -> None:
    ...
    gateway.register_tool("sponsor_get_kpi", lambda p: get_sponsor_kpi(project=p.get("project") or "safari"))
