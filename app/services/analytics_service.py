...
def log_booking_event(
    booking_data: Dict[str, Any], event_type: str = "created"
) -> bool:
    """
    ...
    try:
        ...
        if not sheet_id:
            logger.warning(
                "log_booking_event: no analytics spreadsheet configured; skipping"
            )
            return False
        ...
        ok = log_analytics_event(
            payload, spreadsheet_id=sheet_id, worksheet_name="safari_bookings"
        )
...
def log_api_call(
    ...
    try:
        ...
        if not sheet_id:
            logger.debug(
                "log_api_call: no analytics spreadsheet configured; skipping API metrics write"
            )
            return False
        ...
        ok = log_analytics_event(
            payload, spreadsheet_id=sheet_id, worksheet_name="api_calls"
        )
        if not ok:
    ...
    def after_request(self, response):
        ...
        try:
            if hasattr(g, "start_time"):
                ...
                if request.blueprint == "booking_api" or request.path.startswith(
                    "/api/booking"
                ):
                    user_id = None
                    try:
