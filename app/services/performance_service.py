...
class QueryOptimizer:
    ...
    def get_bookings_with_participants(status: Optional[str] = None, limit: int = 100):
        ...
        """
        query = db.session.query(db.Booking).options(joinedload(db.Booking.participant))
    ...
    def count_active_bookings_by_status() -> Dict[str, int]:
        ...
        results = (
            db.session.query(
                db.Booking.status, func.count(db.Booking.id).label("count")
            )
            .group_by(db.Booking.status)
            .all()
        )
...
class LazyLoadingHelper:
    ...
    def serialize_booking_summary(booking) -> Dict[str, Any]:
        ...
        return {
            ...
            "start_date": (
                booking.start_date.isoformat() if booking.start_date else None
            ),
            "days": booking.days,
...
class CDNConfig:
    ...
    def optimize_image_url(
        ...
        if current_app.config.get("CLOUDINARY_URL"):
            ...
            cdn_url = current_app.config.get(
...
class PerformanceMonitor:
    ...
    def enable_slow_query_logging(threshold_ms: int = 200):
        ...
        def receive_after_cursor_execute(
            ...
            total_time = (
                datetime.now() - conn.info["query_start_time"].pop(-1)
            ).total_seconds() * 1000

            if total_time > threshold_ms:
                logger.warning(f"SLOW QUERY ({total_time:.2f}ms): {statement[:200]}...")
