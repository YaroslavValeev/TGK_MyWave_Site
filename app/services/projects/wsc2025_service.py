...
def save_participant_registration(data: Dict[str, Any]) -> Tuple[bool, Optional[str]]:
    ...
    try:
        ...
        current_app.logger.info(
            f"Участник зарегистрирован: {data.get('full_name')} ({data.get('email')})"
        )
        ...
        current_app.logger.error(
            f"Ошибка сохранения регистрации участника: {e}", exc_info=True
        )
...
def save_coach_registration(data: Dict[str, Any]) -> Tuple[bool, Optional[str]]:
    ...
    try:
        ...
        current_app.logger.info(
            f"Тренер зарегистрирован: {data.get('full_name')} ({data.get('email')})"
        )
        ...
        current_app.logger.error(
            f"Ошибка сохранения регистрации тренера: {e}", exc_info=True
        )
        return False, f"Ошибка при сохранении данных: {str(e)}"
