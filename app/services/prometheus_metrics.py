...
booking_created = Counter(
    "safari_bookings_created_total", "Total number of bookings created", ["status"]
)
...
email_sent = Counter(
    "safari_emails_sent_total", "Total number of emails sent", ["email_type"]
)

cache_hits = Counter("safari_cache_hits_total", "Total number of cache hits")

cache_misses = Counter("safari_cache_misses_total", "Total number of cache misses")
...
active_bookings = Gauge(
    "safari_active_bookings", "Number of active (pending or confirmed) bookings"
)

total_participants = Gauge(
    "safari_total_participants", "Total number of unique participants"
)

system_health = Gauge(
    "safari_system_health", "System health status (1=healthy, 0=unhealthy)"
)
...
def update_metrics():
    ...
    try:
        ...
        with current_app.app_context():
            ...
            logger.debug(
                f"Metrics updated: active_bookings={active_count}, participants={participant_count}"
            )
            return True
