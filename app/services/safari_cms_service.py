...
class SafariCMSService:
    ...
    def sync_routes() -> Dict[str, Any]:
        ...
        for row in rows:
            try:
                ...
                if not doc:
                    doc = Document(
                        ...
                        meta={
                            ...
                            "name": row.get("name", ""),
                            "price": (
                                int(row.get("price", 0)) if row.get("price") else 0
                            ),
                            "duration_days": (
                                int(row.get("duration_days", 0))
                ...
                logger.error(
                    f"Error syncing route {row.get('route_id', 'unknown')}: {e}"
                )
                errors.append(str(e))
        ...
        try:
            ...
            return {"success": True, "count": synced, "errors": errors}
        except Exception as e:
            db.session.rollback()
                ...
                if not doc:
                    doc = Document(
                        ...
                        meta={
                            ...
                            "order": (
                                int(row.get("order", 0)) if row.get("order") else 0
                            ),
                            "synced_at": datetime.utcnow().isoformat(),
                        },
                    )
    ...
    def get_routes() -> List[Dict[str, Any]]:
        ...
        for doc in docs:
            if (
                doc.meta
                and doc.meta.get("type") == "route"
                and doc.meta.get("collection") == SAFARI_COLLECTION_NAME
            ):
                route = {
                    "id": doc.id,
    ...
    def sync_all() -> Dict[str, Any]:
        ...
        total_success = results["routes"].get("success", False) and results["faq"].get(
            "success", False
        )
        total_synced = results["routes"].get("count", 0) + results["faq"].get(
            "count", 0
        )
