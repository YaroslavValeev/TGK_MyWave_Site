...
def _get_lock_by_prefix() -> str:
    ...
    prefix = (
        current_app.config.get("BLOG_PUBLISH_LOCK_BY_PREFIX", "site:mywave")
        if current_app
        else os.getenv("BLOG_PUBLISH_LOCK_BY_PREFIX", "site:mywave")
    )
...
def _get_public_blog_base_url() -> str:
    ...
    try:
        server_name = (
            (current_app.config.get("SERVER_NAME") or "").strip() if current_app else ""
        )
    except Exception:
        server_name = ""
    if server_name:
...
def _safe_find_row_number_by_unique_id(
    ...
    try:
        ...
        logger.error(
            f"[blog-publish] Не удалось перечитать лист для поиска строки по id: {e}",
            exc_info=True,
        )
        return None, None
...
def _find_record_by_sheet_row_number(
    records: List[Dict], sheet_row_number: int
) -> Optional[Dict]:
    """
...
def update_sheet_cells(
    spreadsheet_id: str, sheet_name: str, updates: List[Dict[str, Any]]
) -> bool:
    """
    ...
    try:
        ...
        for update in updates:
            if "range" not in update or "values" not in update:
                continue
            data.append(
                {"range": f"{sheet_name}!{update['range']}", "values": update["values"]}
            )
...
def _read_contract_if_present(
    spreadsheet_id: str, logger=None
...
def acquire_publish_lock(
    row_number: int, lock_ttl_minutes: Optional[int] = None, logger=None
) -> bool:
    """
    ...
    try:
        ...
        if row_number < 2:
            logger.warning(
                f"[blog-publish] Некорректный row_number для lock: {row_number}"
            )
            return False
        ...
        if record is None:
            logger.warning(
                f"[blog-publish] Не найдена запись с row_number={row_number} для lock"
            )
            return False
        ...
        if lock_by_col is not None and lock_until_col is not None:
            ...
            if current_lock_until:
                try:
                    lock_until_dt = datetime.fromisoformat(
                        current_lock_until.replace("Z", "+00:00")
                    )
                    now = datetime.utcnow()
                    if (
                        lock_until_dt > now
                        and current_lock_by
                        and current_lock_by != lock_by
                    ):
                        logger.debug(
                            f"[blog-publish] Lock занят для строки {row_number} (by: {current_lock_by})"
                        )
                        return False
                except Exception:
        ...
        if lock_by_col is not None:
            ...
            updates.append(
                {"range": f"{col_letter}{row_number}", "values": [[lock_by]]}
            )

        if lock_until_col is not None:
            col_letter = _column_index_to_letter(lock_until_col)
            updates.append(
                {"range": f"{col_letter}{row_number}", "values": [[lock_until]]}
            )
        ...
        logger.error(
            f"[blog-publish] Ошибка при получении lock для строки {row_number}: {e}",
            exc_info=True,
        )
        return False
...
def release_publish_lock(row_number: int, logger=None) -> None:
    ...
    try:
        ...
        logger.error(
            f"[blog-publish] Ошибка при снятии lock для строки {row_number}: {e}",
            exc_info=True,
        )
...
def ack_publish(
    ...
    try:
        ...
        if not schema_ok:
            logger.warning(
                f"[blog-publish] Несовпадение схемы Sheets (ack): missing={missing_cols}"
            )
        ...
        if row_number < 2:
            logger.warning(
                f"[blog-publish] Некорректный row_number для ack: {row_number}"
            )
            return False
        ...
        if record is None:
            logger.warning(
                f"[blog-publish] Не найдена запись с row_number={row_number} для ack"
            )
            return False
        ...
        if id_col is not None:
            ...
            if record_id != sheet_id:
                logger.warning(
                    f"[blog-publish] ID не совпадает: ожидали {sheet_id}, получили {record_id}"
                )
                return False
        ...
        if published_posts_col is not None:
            ...
            updates.append({"range": f"{col_letter}{row_number}", "values": [["TRUE"]]})

        # published_at
        if published_at_col is not None:
            col_letter = _column_index_to_letter(published_at_col)
            updates.append(
                {"range": f"{col_letter}{row_number}", "values": [[published_at_str]]}
            )
        ...
        if attempts_col is not None:
            ...
            try:
                current_attempts = int(
                    str(record.get(headers[attempts_col]) or "0").strip() or 0
                )
            except Exception:
                current_attempts = 0
        ...
        sheet_slug = (
            str(record.get(headers[slug_col]) or "").strip()
        ...
        if canonical_url_col is not None:
            ...
            if canonical_url:
                ...
                updates.append(
                    {"range": f"{col_letter}{row_number}", "values": [[canonical_url]]}
                )
        ...
        if review_queue_col is not None:
            ...
            updates.append(
                {"range": f"{col_letter}{row_number}", "values": [["FALSE"]]}
            )
        ...
        if success:
            logger.info(
                f"[blog-publish] Ack отправлен для строки {row_number} (id: {sheet_id})"
            )
        return success

    except Exception as e:
        logger.error(
            f"[blog-publish] Ошибка при отправке ack для строки {row_number}: {e}",
            exc_info=True,
        )
        return False
...
def record_publish_error(
    row_number: int, error_msg: str, increment_attempts: bool = True, logger=None
) -> None:
    """
    ...
    try:
        ...
        if not schema_ok:
            logger.warning(
                f"[blog-publish] Несовпадение схемы Sheets (error write): missing={missing_cols}"
            )
        ...
        record = _find_record_by_row_number(
            records, row_number
        ) or _find_record_by_sheet_row_number(records, row_number)
        if record is None:
            return
        ...
        if error_col is not None:
            ...
            updates.append(
                {"range": f"{col_letter}{row_number}", "values": [[error_msg_short]]}
            )
        ...
        if last_try_col is not None:
            col_letter = _column_index_to_letter(last_try_col)
            updates.append(
                {"range": f"{col_letter}{row_number}", "values": [[now_str]]}
            )

        if updates:
            update_sheet_cells(spreadsheet_id, sheet_name, updates)
            logger.warning(
                f"[blog-publish] Ошибка записана для строки {row_number}: {error_msg[:100]}"
            )

    except Exception as e:
        logger.error(
            f"[blog-publish] Ошибка при записи ошибки для строки {row_number}: {e}",
            exc_info=True,
        )
...
def record_publish_error_by_id(sheet_id: str, error_code: str, logger=None) -> bool:
    ...
    try:
        spreadsheet_id, sheet_name = resolve_parser_source()
        row_number, amb_code = _safe_find_row_number_by_unique_id(
            spreadsheet_id, sheet_name, sheet_id, logger=logger
        )
        if not row_number:
            logger.warning(
                f"[blog-publish] Невозможно безопасно определить строку для ошибки: id={sheet_id}, code={amb_code}"
            )
            return False
        record_publish_error(
            row_number, error_code, increment_attempts=True, logger=logger
        )
        return True
    except Exception as e:
        logger.error(
            f"[blog-publish] Ошибка record_publish_error_by_id: {e}", exc_info=True
        )
        return False
    ...
    try:
        ...
        if not all_exist:
            ...
            if success and created:
                logger.info(
                    f"[blog-publish] Автоматически созданы колонки: {', '.join(created)}"
                )
            elif not success:
                logger.warning(
    ...
    try:
        ...
        try:
            ...
            if contract_data:
                ...
                logger.debug(
                    "[blog-publish] Лист CONTRACT отсутствует — работаем по headers raw_feed"
                )
        except Exception as e:
        ...
        logger.info(
            f"[blog-publish] Проверка {len(records)} записей на готовность к публикации"
        )
        ...
        if not schema_ok:
            logger.warning(
                f"[blog-publish] Несовпадение схемы Sheets (publish_ready_posts): missing={missing_cols}"
            )
            # Собираем статистику по отсутствующим колонкам
            for col in missing_cols:
                schema_mismatch_missing_cols[col] = (
                    schema_mismatch_missing_cols.get(col, 0) + 1
                )
        ...
        for index, row in enumerate(records):
            ...
            logger.debug(
                f"[blog-publish] Найдена запись READY_TO_PUBLISH: id={sheet_id}, row={index+2}"
            )
            ...
            if published_posts in ("TRUE", "1", "YES", "ДА"):
                ...
                stats["skipped"] += 1
                logger.debug(
                    f"[blog-publish] Запись {sheet_id} уже опубликована (published_posts=TRUE)"
                )
                continue
            ...
            if scheduled_at and scheduled_at > now:
                ...
                stats["skipped"] += 1
                logger.debug(
                    f"[blog-publish] Запись {sheet_id} запланирована на будущее: {scheduled_at}"
                )
                continue
            ...
            if not row_number:
                ...
                logger.warning(
                    f"[blog-publish] row_number отсутствует/невалиден для id={sheet_id}: {code}"
                )
            ...
            if not acquire_publish_lock(row_number, logger=logger):
                stats["locked"] += 1
                logger.debug(
                    f"[blog-publish] Не удалось получить lock для записи {sheet_id} (строка {row_number})"
                )
                continue
            ...
            try:
                ...
                if (
                    post
                    and post.checksum == checksum
                    and post.status in ("PUBLISHED", "published")
                ):
                    ...
                    final_posts = str(
                        row.get("final_posts") or row.get("text") or ""
                    ).strip()
                    ...
                    tags = _parse_tags(
                        row.get("raw_tags") or row.get("tags"), row.get("ne")
                    )
                    ...
                    post.teaser = (
                        excerpt or ""
                    )  # teaser не может быть None из-за ограничения БД

                    cover = str(
                        row.get("cover_image_url") or row.get("image_url") or ""
                    ).strip()
                    if not cover:
                    ...
                    media_html = _embed_media_from_json(
                        row.get("media_json"), exclude_url=cover
                    )
                    if media_html:
                ...
                if ack_publish(
                    row_number, sheet_id, published_at, slug=slug, logger=logger
                ):
                    stats["published"] += 1
                    successful_acks += 1
                    logger.info(
                        f"[blog-publish] Пост {sheet_id} опубликован (строка {row_number})"
                    )
                else:
                    ...
                    stats["published"] += 1
                    logger.warning(
                        f"[blog-publish] Пост {sheet_id} опубликован в БД, но ack в Sheets не отправлен"
                    )
                ...
                error_msg = str(e)
                record_publish_error(
                    row_number, error_msg, increment_attempts=True, logger=logger
                )
                release_publish_lock(row_number, logger=logger)
                stats["failed"] += 1
                logger.error(
                    f"[blog-publish] Ошибка публикации поста {sheet_id}: {e}",
                    exc_info=True,
                )
        ...
        if total_p0_errors > 0:
            error_details = ", ".join(
                [
                    f"{code}: {count}"
                    for code, count in p0_error_counts.items()
                    if count > 0
                ]
            )
            logger.warning(
        ...
        if schema_mismatch_missing_cols:
            top_missing = sorted(
                schema_mismatch_missing_cols.items(), key=lambda x: -x[1]
            )[:5]
        ...
        if status_counts:
            ...
            logger.info(
                f"[blog-publish] Распределение статусов (топ-10): {status_list}"
            )
        ...
        logger.error(
            f"[blog-publish] Критическая ошибка при публикации готовых постов: {e}",
            exc_info=True,
        )
        return stats
