    ...
    try:
        ...
        try:
            _sheets.spreadsheets().get(
                spreadsheetId=current_app.config.get("SPREADSHEET_ID")
            ).execute()
        ...
        try:
            ...
            with eventlet.Timeout(8, False):  # если зависает дольше 8с — прерываем
                service[2].events().insert(
                    calendarId=current_app.config["GOOGLE_CALENDAR_ID"], body=event_body
                ).execute()
...
def upload_to_drive(service, file_obj, user_id, folder_id):
    try:
        ...
        media = MediaIoBaseUpload(
            file_obj.stream, mimetype=file_obj.content_type, resumable=True
        )
        uploaded_file = (
            service[0]
            .files()
...
def list_user_files(service, folder_id, user_id):
    try:
        query = (
            f"'{folder_id}' in parents and trashed=false and name contains '{user_id}_'"
        )
        results = (
            service[0]
            .files()
...
def upload_file_to_drive(file_content: bytes, filename: str):
    ...
    if not creds_path or not folder_id:
        raise ValueError(
            "Отсутствуют переменные окружения: GOOGLE_SERVICE_ACCOUNT_FILE или DRIVE_FOLDER_ID"
        )
