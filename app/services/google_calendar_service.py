...
def _get_calendar_service():
    ...
    try:
        ...
        logger.debug(
            "Could not read service account file (%s) to extract client_email: %s",
            sa_path,
            e,
        )

    logger.info(
        "Calendar credentials: client_email=%s scopes=%s",
        client_email or getattr(creds, "service_account_email", None),
...
def create_event(date: str, time: str, duration_minutes: int = 60) -> dict:
    ...
    calendar_id = current_app.config.get(
        "GOOGLE_CALENDAR_ID"
    ) or current_app.config.get("CALENDAR_ID")
    ...
    try:
        ...
        try:
            content = (
                ...
                or getattr(e, "error_details", None)
                or str(e)
            )
        except Exception:
            content = str(e)
        logger.error(
            "Failed to create calendar event: HttpError status=%s content=%s",
            status,
            content,
        )
        raise
