...
class YooKassaPaymentProcessor:
    ...
    def create_payment(
        self, amount: float, description: str, return_url: str, **kwargs
    ) -> Dict:
    ...
    def refund_payment(self, payment_id: str, amount: Optional[float] = None) -> Dict:
        ...
        logger.info(
            f"[PLACEHOLDER] Refunding payment {payment_id}: refund_id={refund_id}"
        )
...
def handle_payment_webhook(payload: Dict) -> Dict:
    ...
    return {"status": "acknowledged", "payment_id": payment_id, "event": event_type}
