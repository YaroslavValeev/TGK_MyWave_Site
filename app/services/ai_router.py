...
def get_user_chat_history(client_id):
    ...
    try:
        ...
        missing_configs = [
            config for config in required_configs if not current_app.config.get(config)
        ]
        if missing_configs:
        ...
        if not all(col in records[0] for col in required_columns):
            logger.error(
                f"Отсутствуют необходимые колонки. Требуются: {required_columns}"
            )
            return []
