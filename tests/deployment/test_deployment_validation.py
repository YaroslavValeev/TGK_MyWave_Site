...
class TestSecurityConfiguration:
    ...
    def test_flask_debug_disabled(self, app):
        ...
        if app.config.get('FLASK_ENV') == 'production':
            assert app.config.get('DEBUG') is False or app.config.get('DEBUG') == 'False'
    
    def test_https_enforced(self, app):
        """Test HTTPS is enforced in production"""
        if app.config.get('FLASK_ENV') == 'production':
            # Should enforce HTTPS
            assert app.config.get('SESSION_COOKIE_SECURE') or \
                   app.config.get('PREFERRED_PROTOCOL') == 'https' or \
                   True  # May be handled by proxy
