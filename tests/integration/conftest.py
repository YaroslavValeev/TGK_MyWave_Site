...
def configure_logging_for_tests():
    ...
    stream_handler.setLevel(
        logging.WARNING
    )  # Only log warnings and above to reduce noise
...
def _mock_create_workout_if_not_exists(
    date_str, time_str, showcase_id=None, slot_type=None
):
...
def stub_google_helpers(monkeypatch):
    ...
    monkeypatch.setattr(
        "app.modules.sheets_access.append_dict_to_sheet", _mock_append_dict_to_sheet
    )
    monkeypatch.setattr(
        "app.modules.sheets_access.update_sheet_row", _mock_update_sheet_row
    )
    monkeypatch.setattr(
        "app.modules.sheets_access.get_google_sheet", _mock_get_google_sheet
    )
    ...
    for fn_name in (
        ...
        try:
            monkeypatch.setattr(
                f"{gs}.{fn_name}", globals().get(f"_mock_{fn_name}", lambda *a, **k: [])
            )
        except Exception:
    ...
    monkeypatch.setattr(
        "app.modules.calendar_integration.create_workout_if_not_exists",
        _mock_create_workout_if_not_exists,
    )
    ...
    monkeypatch.setattr(
        "app.modules.calendar_integration.create_calendar_event", _calendar_call_through
    )
    # also patch alternate names used elsewhere
    try:
        monkeypatch.setattr(
            "app.modules.calendar_integration.create_event", _mock_create_calendar_event
        )
    except Exception:
        pass
    try:
        monkeypatch.setattr(
            "app.modules.calendar_integration.get_google_calendar_service",
            lambda *a, **k: None,
        )
    except Exception:
        pass
    ...
    try:
        monkeypatch.setattr(
            "app.services.google_calendar_service.create_event",
            _mock_google_calendar_create_event,
        )
    except Exception:
        pass
    ...
    try:
        monkeypatch.setattr(
            "app.services.google.add_event_to_calendar", _mock_add_event_to_calendar
        )
    except Exception:
        pass
    ...
    try:
        monkeypatch.setattr(
            "app.services.google.get_google_services", lambda: (None, None, None)
        )
    except Exception:
        pass
...
def auth_headers():
    ...
    return {"Authorization": "Bearer None", "Content-Type": "application/json"}
