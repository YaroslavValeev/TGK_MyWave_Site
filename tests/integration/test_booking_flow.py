...
class TestBookingFlow:
    ...
    def test_booking_creation_authenticated(self, client, auth_headers):
        ...
        response = client.post(
            "/api/bookings", data=json.dumps(booking_data), headers=auth_headers
        )
...
class TestPaymentFlow:
    ...
    def test_payment_failure(self, mock_payment, client, auth_headers):
        ...
        response = client.post(
            ...
            data=json.dumps({"amount": 50000, "payment_method": "yookassa"}),
            headers=auth_headers,
        )
...
class TestNotificationFlow:
    ...
    def test_booking_confirmation_email(self, mock_email, client, auth_headers):
        ...
        if mock_email.called:
            assert (
                "booking" in str(mock_email.call_args).lower()
                or "confirmation" in str(mock_email.call_args).lower()
            )
...
class TestSecurityValidation:
    ...
    def test_cors_validation(self, client, auth_headers):
        ...
        response = client.options(
            "/api/bookings", headers={"Origin": "https://example.com"}
        )

        # Check for CORS headers
        assert (
            "Access-Control-Allow-Origin" in response.headers
            or response.status_code in [200, 204, 404]
        )
