...
class TestAPIEndpoints:
    ...
    def test_payment_endpoint(self, client):
        ...
        response = client.post('/api/bookings/1/payment', json={
            'amount': 10000,
...
class TestConcurrency:
    ...
    def test_multiple_booking_creations(self, client, app):
        ...
        with app.app_context():
            ...
            for i in range(5):
                response = client.post('/api/bookings', json={
                    'start_date': (datetime.now() + timedelta(days=7+i)).isoformat(),
                    'end_date': (datetime.now() + timedelta(days=14+i)).isoformat(),
                    'num_participants': 2
                })
