...
def live_server():
    ...
    try:
        ...
        cal.get_available_slots = lambda date: [{'time': '10:00', 'available': True, 'remaining': 3}]
    except Exception:
        pass
    try:
...
def test_booking_flow(page, live_server):
    ...
    try:
        page.wait_for_selector('#success-modal, .success-modal, .toast-success', timeout=5000)
    except Exception:
        pass

    # Assert that either success-modal or success toast is present
    assert page.query_selector('#success-modal') or page.query_selector('.toast-success')
