...
class TestDockerfileStructure:
    ...
    def test_dockerfile_multi_stage(self):
        ...
        assert "COPY --from=builder" in content, "Dockerfile must copy from builder stage"
    ...
    def test_dockerfile_healthcheck(self):
        ...
        assert "/api/metrics/health" in content, "Health check must verify metrics endpoint"
...
class TestDockerCompose:
    ...
    def test_db_service_config(self):
        ...
        # Check image
        assert "postgres" in db_service.get("image", "").lower(), "DB must use PostgreSQL"
        
        # Check environment variables
        env = db_service.get("environment", {})
        assert "POSTGRES_USER" in env or any("POSTGRES_USER" in str(v) for v in env.values()), \
            "DB must define POSTGRES_USER"
        assert "POSTGRES_PASSWORD" in env or any("POSTGRES_PASSWORD" in str(v) for v in env.values()), \
        ...
        assert "pgdata" in db_service.get("volumes", []) or \
               any("pgdata" in str(v) for v in db_service.get("volumes", [])), \
    ...
    def test_redis_service_config(self):
        ...
        # Check image
        assert "redis" in redis_service.get("image", "").lower(), "Redis service must use Redis image"
        
        # Check health check
        assert "healthcheck" in redis_service, "Redis service must have healthcheck"
        
        # Check volume
        assert "redis_data" in redis_service.get("volumes", []) or \
               any("redis_data" in str(v) for v in redis_service.get("volumes", [])), \
    ...
    def test_web_service_config(self):
        ...
        # Check build context
        assert "build" in web_service or "image" in web_service, \
            "Web service must have build or image configuration"
        
        # Check environment
        env = web_service.get("environment", {})
        assert any("DATABASE_URL" in str(k) or "DATABASE_URL" in str(v) for k, v in env.items()), \
    ...
    def test_docker_compose_version(self):
        ...
        assert version and version >= "3.8", "docker-compose version must be 3.8 or higher"
...
class TestDockerConfig:
    ...
    def test_nginx_conf_valid(self):
        ...
        assert "rate limiting" in content.lower() or "limit_req" in content, \
...
class TestHealthCheckEndpoint:
    ...
    def test_health_check_returns_json(self):
        ...
        assert "health_check" in content or "health" in content.lower(), \
            "metrics_api.py must define health check endpoint"
        assert "json" in content.lower() or "return" in content, \
...
class TestDockerSecurity:
    ...
    def test_dockerfile_no_root_user(self):
        ...
        assert user_index > copy_index, "USER directive must come after copying application code"
