...
def test_api_key_enforced_and_validated(client, app):
    ...
    # Wrong key -> 401
    r = client.post('/api/ai/gateway/message', json={'message': 'hi'}, headers={'Authorization': 'Bearer badkey'})
    assert r.status_code == 401

    # Correct key -> 200
    r = client.post('/api/ai/gateway/message', json={'message': 'hi'}, headers={'Authorization': 'Bearer goodkey'})
