...
def test_get_slots_tool(monkeypatch, client):
    ...
    gateway.tools["get_available_slots"] = (
        {"def": entry.get("def"), "fn": fake_get_slots_tool}
        if isinstance(entry, dict)
        else {"def": None, "fn": fake_get_slots_tool}
    )

    resp = post_message(
        client,
        '__call_tool__:get_available_slots:{"service_id":"svc_wsc","date":"2025-12-01"}',
    )
    assert resp.status_code == 200
    data = resp.get_json()
    assert data.get("type") == "tool_result"
...
def test_create_booking_tool(monkeypatch, client):
    ...
    gateway.tools["create_booking"] = (
        {"def": entry.get("def"), "fn": fake_create_booking}
        if isinstance(entry, dict)
        else {"def": None, "fn": fake_create_booking}
    )
