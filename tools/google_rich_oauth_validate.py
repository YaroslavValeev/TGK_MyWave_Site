...
def run_oauth_and_validate(client_secrets_path, port=52002, no_auto_port=False):
        ...
        try:
            ...
            if not no_auto_port:
                ...
                try:
                    ...
                    print('Falling back to manual console flow. Follow the printed URL and paste the code here.')
            else:
                print('Auto port selection is disabled (--no-auto-port). Falling back to manual console flow. Follow the printed URL and paste the code here.')
                ...
                try:
                    ...
                    maybe = flow.authorization_url(access_type='offline', prompt='consent')
                    ...
                    try:
                        ...
                        print('Cannot build authorization URL automatically:', str(e_auth))

                if not auth_url:
                    raise RuntimeError('Unable to build authorization URL for manual flow.')
    ...
    try:
        masked = (creds.token[:10] + '...' + creds.token[-6:]) if creds.token and len(creds.token) > 20 else creds.token
        print('Access token (masked):', masked)
        try:
            ti = requests.get('https://www.googleapis.com/oauth2/v3/tokeninfo', params={'access_token': creds.token}, timeout=10)
        ...
        try:
            token_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'tmp_token.txt'))
            with open(token_path, 'w', encoding='utf-8') as tf:
                tf.write(creds.token or '')
            print(f'Full access token saved to: {token_path} (local file, KEEP PRIVATE)')
            ...
            curl_cmd = (
                ...
                "-d '{""html"": ""<html><head></head><body>test</body></html>"", ""requestScreenshot"": false}'"
            )
...
def main():
    ...
    p.add_argument('--client-secrets', required=True, help='Path to client_secret.json (OAuth client ID)')
    p.add_argument('--port', type=int, default=52002, help='Local redirect port')
    p.add_argument('--no-auto-port', action='store_true', help='Do not try a free port if the requested port is busy; use manual flow instead')
    args = p.parse_args()
    ...
    try:
        run_oauth_and_validate(args.client_secrets, port=args.port, no_auto_port=args.no_auto_port)
    except Exception as exc:
