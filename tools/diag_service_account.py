...
if exists:
    try:
        ...
        try:
            ...
            req = Request()
            creds.refresh(req)
            out.append(f"refresh_ok=True token_len={len(creds.token) if creds.token else 0}")
        ...
        try:
            ...
            if pk:
                key = serialization.load_pem_private_key(pk.encode('utf-8'), password=None, backend=default_backend())
...
with open(os.path.join(os.path.dirname(__file__), '..', 'tmp_service_account_diag.txt'), 'w', encoding='utf-8') as f:
    f.write('\n'.join(out))
