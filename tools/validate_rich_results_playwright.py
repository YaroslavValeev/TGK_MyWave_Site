...
try:
    ...
    print('Missing dependency playwright. Install with: pip install playwright && playwright install')
    sys.exit(2)
...
def run_playwright(html, headless=True, slow_mo=50, dump_controls=False):
    with sync_playwright() as p:
        ...
        if not clicked:
            ...
            try:
                elems = page.query_selector_all('button, [role="button"], [role="tab"], a')
                samples = []
                for e in elems[:40]:
                    try:
                        txt = e.inner_text().strip()
                        ...
                        continue
                print('Could not find Test code button automatically, opening page and waiting for manual paste...')
        ...
        if not set_ok:
            print('Textarea/CodeMirror not found or could not be set programmatically. Trying additional heuristics...')

            # Heuristic 1: search for likely editor containers by class/name and try to set innerText/value
            editor_candidates = [
                '.CodeMirror', '.ace_editor', '.monaco-editor', '.editor', '.source-editor',
            ...
            for sel in editor_candidates:
                try:
                    ...
                    for e in elems:
                        try:
                            # try setting .value
                            page.evaluate('(el, html)=>{ try{ el.value = html; el.innerText = html; el.textContent = html; return true;}catch(e){return false;} }', e, html)
                            # try specialized CodeMirror API on element
                            try:
                                page.evaluate('(el, html)=>{ if(el.CodeMirror){ el.CodeMirror.setValue(html); return true; } return false; }', e, html)
                            except Exception:
                                pass
        ...
        if not set_ok:
            ...
            try:
                ...
                print('Injected overlay textarea with id playwright_inject. Please copy its contents into the Test code editor, then press Run.')
                print('Leaving browser open for 180 seconds...')
                time.sleep(180)
                browser.close()
                return None
            except Exception:
                print('Failed to inject overlay textarea; giving up and leaving the page for manual paste.')
        ...
        try:
            ...
            try:
                ...
                if ok:
                    print('Set underlying textarea[jsname="bqeLof"] or CodeMirror textarea and dispatched input/change events.')
            except Exception:
                pass
        except Exception:
            pass
        ...
        run_jsname_candidates = ['div[jsname="oe2Hje"]', 'div[jsname="iyDKIb"]', 'div[jsname="LZQqje"]', 'div[jsname="oe2Hje"][role="button"]']
        clicked_run = False
                    ...
                    if ad in (None, 'false'):
                        try:
                            ...
                            try:
                                ...
                                print('Clicked run button via JS click on selector:', sel)
                                clicked_run = True
                                break
                            except Exception:
                                pass
        ...
        if not clicked_run:
            ...
            for rb in run_buttons:
                try:
                    ...
                    continue
        if not clicked_run:
            print('Failed to click Run Test button automatically - you may need to press it manually.')
        ...
        out_png = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'tmp_rich_results.png'))
...
def main():
    ...
    if res.get('html'):
        out_html = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'tmp_rich_results_page.html'))
