...
try:
    ...
    out['Server_dir'] = [n for n in dir(Server) if not n.startswith('__')] if Server else None
    # try instantiate
    try:
        inst = Server('inspect')
...
try:
    ...
    # list callables
    out['stdio_callables'] = [n for n,v in inspect.getmembers(stdiomod, inspect.isfunction)]
except Exception as e:
