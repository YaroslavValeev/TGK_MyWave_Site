...
def basic_checks(info):
    ...
    if info.get("type") != "service_account":
        return (
            False,
            f"Unexpected credential type: {info.get('type')}. Expected 'service_account'",
        )
    pk = info.get("private_key", "")
    if "BEGIN PRIVATE KEY" not in pk:
        return (
            False,
            "private_key does not look like a PEM block (missing BEGIN PRIVATE KEY)",
        )
...
def try_construct_credentials(info):
    ...
    try:
        ...
        return (
            True,
            f"Credentials object created (scopes not applied). Service account: {getattr(creds, 'service_account_email', 'unknown')}",
        )
    except Exception as e:
...
def main():
    ...
    if not path:
        print(
            "Service account JSON not found. Checked common locations and GOOGLE_SERVICE_ACCOUNT_FILE env var."
        )
    ...
    if not ok2:
        print(
            "If you intend to use Google APIs, install google-auth (pip install google-auth google-auth-oauthlib google-auth-httplib2)"
        )
        sys.exit(5)

    print("\nService account JSON appears valid for local usage. Next steps:")
    print(
        " - Ensure the target Spreadsheet is shared with the service account email (client_email)."
    )
    print(
        " - Set environment variable SPREADSHEET_ID to the target spreadsheet id or add it to app config."
    )
    print("\nOptional test (requires network & googleapiclient):")
    print(
        "  python -c \"from google.oauth2 import service_account; from googleapiclient.discovery import build; creds=service_account.Credentials.from_service_account_file('"
        + path.replace("\\", "/")
        + "'); svc=build('sheets', '"
        + "v4"
        + '\', credentials=creds); print(svc.spreadsheets().get(spreadsheetId="<SPREADSHEET_ID>").execute())"'
    )
