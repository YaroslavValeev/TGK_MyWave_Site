...
API_KEY = (
    ...
    or os.environ.get("GOOGLE_API_KEY".lower())
    or os.environ.get("GOOGLE_API_KEY".upper())
)
if not API_KEY:
...
if not API_KEY:
    print(
        "ERROR: GOOGLE_API_KEY environment variable not found. Please set it (or add to .env)."
    )
    sys.exit(2)
...
for body in bodies:
    try:
        print(
            "Posting to Google Rich Results Test API with body keys:", list(body.keys())
        )
        resp = session.post(
            ENDPOINT, headers=headers, data=json.dumps(body), timeout=30
        )
