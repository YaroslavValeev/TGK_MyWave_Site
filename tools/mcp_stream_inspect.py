...
stdio = importlib.import_module('mcp.server.stdio') if importlib.util.find_spec('mcp.server.stdio') else None
...
SessionMessage = getattr(low, 'SessionMessage', None) or getattr(stdio, 'SessionMessage', None)
...
print('\nstdio module members:', getattr(stdio, '__file__', None) if stdio else 'stdio not present')
if stdio:
