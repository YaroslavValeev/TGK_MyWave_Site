...
from blog_health_notify import run_blog_health
...
async def cmd_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = (
        ...
        "/blog_health — smoke-check источника блога (raw_feed)\n"
...
async def cmd_blog_health(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await guard(update):
        return
    cwd = runner_cwd()
    try:
        rc, text = run_blog_health(cwd)
    except subprocess.TimeoutExpired:
        await update.message.reply_text("❌ blog_health: timeout")
        return
    await update.message.reply_text(text)
...
def main():
    ...
    app.add_handler(CommandHandler("blog_health", cmd_blog_health))
